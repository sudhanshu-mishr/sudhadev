            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-3xl overflow-hidden shadow-2xl transition-all duration-300",
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    on
                      ? "bg-blue-600 shadow-[0_0_10px_rgba(37,99,235,1)]"
...
function DefaultFanCard({ item, active }: { item: CardStackItem; active: boolean }) {
  return (
    <GlowCard 
      ...
      customSize 
      className="w-full h-full border-0 rounded-none bg-transparent backdrop-blur-0 shadow-none"
    >
      <div className="relative h-full w-full">
          ...
          {item.imageSrc ? (
            <img
              ...
              className={cn(
                ...
                !active ? "grayscale opacity-50" : "grayscale-0 opacity-80"
              )}
              draggable={false}
              loading="eager"
            />
          ) : (
            <div className="flex h-full w-full items-center justify-center bg-slate-900/50 text-sm text-slate-500 italic">
              No image
            </div>
          )}
        </div>
        ...
        <div className="relative z-10 flex h-full flex-col justify-end p-8">
          <div className="truncate text-3xl font-black text-white uppercase italic tracking-tighter">
            {item.title}
          </div>
          {item.description ? (
            <div className="mt-3 line-clamp-2 text-sm text-slate-300 font-medium leading-relaxed">
              {item.description}
            </div>
          ) : null}
        </div>
